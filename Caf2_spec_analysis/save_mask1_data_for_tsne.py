...
def main():
    foldername_list = ['caf2_col1','caf2_col1_4_8020/'] # '1000','9010', '8020','7030' 
    filename_list = ['LMT_1','LMT_2','LMT_3'] #'LMT_1','LMT_2','LMT_3','LMT_4']
    for foldername in foldername_list:
        for filename in filename_list:
            ...
            after_collagen = f'W:/3. Students/Tianyi/03232026_colle1+4/{foldername}/{filename}'+'.mat'
            save_path = f'../res/03232026_colle1+4/org/{foldername}/{filename}/'
            save_2nd = f'../res/03232026_colle1+4/2nd/{foldername}/{filename}/'
            ...
            for i in range(data.shape[0]):
                for j in range(data.shape[1]):
                    ...
                    max_amide1 = np.max(amide1_band)
                    # print(max_amide1)
                    if 0.18 <= max_amide1 <= 0.24:
                        valid_spectra.append(spectrum)

            print(f"Found {len(valid_spectra)} spectra with Amide I max between 0.16 and 0.24")

            # Randomly sample up to 10,000
            random.seed(42)
            n_samples = min(len(valid_spectra), 500)
            selected_spectra = random.sample(valid_spectra, n_samples)
